...
- git diff --check: pass; semantic assertion: 20 required markers, 0 stale markers, 72 balanced fences, 31 sections, AC-01..AC-30, unchanged status
- node .agentplane/policy/check-routing.mjs: policy routing OK
- ap doctor: OK, errors=0 warnings=0
- Official WB search/browser evidence: token purposes and token-dependent limits; promotion OpenAPI cluster DELETE/status 4; release notes canceled semantics
...
- Direct curl to dev.wildberries.ru returns portal-specific HTTP 498; canonical links and claims were therefore validated through the official indexed browser/search surface.
