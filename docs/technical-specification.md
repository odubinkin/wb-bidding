...
## Как пользоваться техническим заданием

Это нормативный документ: он фиксирует требования, границы и критерии приёмки реализации. Его
детализация полезна после первого знакомства с проектом, но неудобна как вводный учебник. Если
вы пришли извне, начните с [путеводителя](project-guide.md), затем прочитайте
[архитектуру](architecture.md) и вернитесь к разделам ТЗ, которые нужны для вашей роли.

В ТЗ встречаются английские имена API, моделей и статусов — это точные идентификаторы реализации
или внешнего контракта. Их смысл дан при первом использовании в [путеводителе](project-guide.md),
[модели данных](data-model.md) и профильных документах. Требование «fail-closed» означает не
аварийное падение, а предсказуемый отказ от рискованной денежной операции, пока её условия не
подтверждены.
