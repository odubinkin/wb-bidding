...
## От фактов к предложению ставки

Этот документ описывает библиотеку, которая получает уже проверенные данные и возвращает
`Decision` — неизменяемое предложение действия с объяснением. Она **не** ходит в WB и не меняет
ставку: применение выполняет отдельный [конвейер записи](write-pipeline.md). Такое разделение
даёт два свойства: расчёт можно повторить на тех же снимках, а сетевой сбой не меняет ответ
алгоритма.

Прежде чем углубляться в формулы, прочитайте сквозной пример и словарь в
[путеводителе](project-guide.md), затем детальный [алгоритм управления ставками](bidding-algorithm.md).
Здесь `policy` означает версию ограничений бизнеса, `economics` — маржу товара, а `blocker` —
объяснимую причину, по которой рискованное действие не разрешено. Блокер — ожидаемый результат
защиты, а не обязательно программная ошибка.
