...
Нормативная реализация находится в пакете `decision-engine`; подробные формулы и данные
описаны в [документе модуля принятия решений](decision-engine.md). Модуль не выполняет I/O и получает
