...
Это формальное описание того, как система выбирает или отклоняет предложение новой ставки. Оно
предполагает, что читатель уже знает базовую историю: ставка относится к конкретному target,
данные WB сначала становятся snapshots и evidence, а `Decision` ещё не является сетевой записью.
Если это не так, начните со [сквозного примера](project-guide.md#сквозной-пример) и вернитесь
сюда после [архитектуры](architecture.md).

Слово «оптимальная» ниже всегда означает _лучшая среди разрешённых и достаточно доказанных
кандидатов по текущей версии policy_, а не обещание глобальной прибыли. Консервативность,
блокировки и ограничение скорости изменения — часть алгоритма: они уменьшают вред от неполной
статистики и неизвестной семантики WB, даже если иногда сохраняют неидеальную ставку.
