...
## Что имитирует mock и чего он не доказывает

Mock — локальный сервер, который воспроизводит утверждённую модель WB API для тестов. В отличие
от настоящей WB он управляется тестом: у него есть виртуальные часы, заранее заданные ответы,
ошибки и задержка видимости ставки. Благодаря этому один и тот же E2E-сценарий можно повторить
без реального кабинета и без расхода бюджета. Общее различие режимов приведено в
[путеводителе](project-guide.md#режимы-работы).

Успешный тест с mock подтверждает нашу реализацию против pinned контракта, но не доказывает, что
production WB ведёт себя так же. Для этого существуют профиль интеграции, evidence и правила
fail-closed из [интеграции WB API](wb-api-integration.md). Не используйте служебные маршруты
`/__mock` как часть продукта: они нужны только для подготовки и наблюдения тестовых сценариев.
