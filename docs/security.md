...
Сервис хранит доступ к рекламному кабинету и способен менять денежно значимую ставку. Поэтому
безопасность здесь включает не только секреты: нужно не дать запросу попасть к неправильному
хосту, связать deployment с одним продавцом, ограничить права административного API и закрыть
запись при неполном подтверждении. Контекст ролей, токена, binding и режимов работы дан в
[путеводителе](project-guide.md); конкретные HTTP-контракты — в
[интеграции WB API](wb-api-integration.md).

Несколько gates — это сознательная защита от одной ошибочной настройки. Например, включённый
флаг write не обходит kill switch, неподтверждённый API-профиль или несоответствие seller binding.
Последствие такого дизайна: оператор может увидеть отказ вместо удобного «best effort», но
система не начнёт списывать рекламный бюджет по предположению.
