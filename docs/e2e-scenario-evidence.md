# Трассировка обязательных поведенческих сценариев

## Что доказывают эти сценарии

Каждая строка связывает поведенческое требование с тестом, который можно запустить. E2E здесь
означает сквозной путь через несколько компонентов (например, sync → decision → очередь → mock
WB), а не обязательный доступ к настоящему кабинету. Чтобы понять, почему это различие важно,
прочитайте [путеводитель](project-guide.md#режимы-работы) и [тестирование](testing.md).

Таблица не заменяет результат запуска и не утверждает, что положительная production-ветвь
подтверждена там, где контракт WB намеренно закрыт fail-closed. Для общей картины готовности
сопоставляйте её с [матрицей приёмочных доказательств](acceptance-evidence.md) и
[реестром расхождений](implementation-deviations.md).

Статус `AUTOMATED` означает, что поведение проверяется исполняемым тестом, но сам по себе не
означает системный E2E. Уровень evidence определяется исполняемым файлом в строке: `tests/unit`,
`tests/property` и `tests/golden` проверяют чистую логику; `tests/contract` — HTTP/schema boundary;
`tests/integration` — несколько модулей и PostgreSQL; только `tests/e2e` доказывает системный путь
mock HTTP → durable state → reconciliation. `PARTIAL` означает,
...
Локально автоматизированы все 51 обязательная поведенческая строка на явно указанном в evidence
уровне; это не означает наличие 51 системного E2E. Production cluster contract
