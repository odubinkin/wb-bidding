...
Runbook предназначен для оператора во время инцидента. Он не объясняет устройство с нуля:
сначала прочитайте [путеводитель](project-guide.md), [конфигурацию](configuration.md) и
[наблюдаемость](observability.md). Ниже `queue`, `UNKNOWN`, `reconciliation`, `kill switch` и
`readiness` имеют значения, определённые там и в [конвейере записи](write-pipeline.md).

Каждая процедура стремится сначала остановить рискованное изменение, затем сохранить факты для
разбора и только потом восстановить обработку. Не удаляйте строки очереди и снимки «для
разблокировки»: именно они позволяют отличить уже применённую WB ставку от неотправленного
намерения. Если безопасный ответ неизвестен, оставьте writes выключенными и эскалируйте ситуацию.
