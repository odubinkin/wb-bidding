...
Это реестр проверяемых оснований для утверждений о готовности, а не список возможностей продукта.
Перед чтением полезно понимать, что локальный mock-тест, production WB и решение владельца
продукта доказывают разные вещи; это объяснено в [путеводителе](project-guide.md) и
[тестировании](testing.md). Слово _evidence_ означает ссылку на воспроизводимую команду, тест,
артефакт или внешнее решение, а не устное подтверждение.

`AUTOMATED` означает, что требование покрыто исполняемой проверкой. `LOCAL VERIFIED` добавляет
конкретный локальный запуск. `EXTERNAL GATE` означает, что локальный код не может честно заменить
условие: например, реальный WB sandbox, hosted CI или решение уполномоченного владельца. Такой
статус не является дефектом документации и не должен маскироваться как «готово».
