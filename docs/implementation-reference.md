...
## Что этот справочник объясняет

Это подробное продолжение [архитектуры](architecture.md), а не первая точка знакомства. Он
отвечает на вопрос «какой компонент выполняет шаг и что сохраняется между шагами». Перед чтением
нужно различать источник данных WB, сохранённый snapshot, расчётное decision и элемент очереди:
их определения и пример полного пути приведены в [путеводителе](project-guide.md).

Каждый раздел ниже связывает наблюдаемое поведение с кодом. Слова `read`, `write`, `job`,
`lease`, `reconciliation` и `fail-closed` обозначают конкретные механизмы, а не общие обещания:
read получает факт из внешней системы, write пытается изменить её, job — плановая работа,
lease — временное право одного исполнителя обработать очередь, reconciliation — доказательная
сверка результата, а fail-closed — запрет опасного пути при неизвестном условии.
