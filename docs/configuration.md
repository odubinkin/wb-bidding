...
Конфигурация описывает не только адреса и пароли, но и границу между безопасным наблюдением и
реальным изменением денег. Начните с [режимов работы](project-guide.md#режимы-работы): `mock`
предназначен для тестов, а `production` по умолчанию остаётся read-only. Переменные ниже читаются
при старте и валидируются до запуска фоновых jobs; неверное или неполное значение должно закрыть
опасную возможность, а не заменить её значением по умолчанию.

В примерах «токен» означает секрет, выданный WB, а «binding» — сохранённое соответствие между
этим развёртыванием и одним кабинетом продавца. Не добавляйте реальные значения в `.env.example`,
Git, логи или запросы в Swagger. Полная последовательность включения writes находится в
[безопасности](security.md), а назначение конкретных переменных — в таблицах ниже.
