...
## Зачем синхронизация отделена от расчёта

WB возвращает текущие ставки и статистику во внешнем API, но этот ответ может быть частичным,
запаздывающим или исправленным позднее. Поэтому система не передаёт «свежий HTTP JSON» прямо в
алгоритм. Сначала она сохраняет снимок, нормализует его и проверяет, хватает ли набора данных для
evidence — доказательной основы решения. Термины snapshot, evidence и target введены в
[путеводителе](project-guide.md); последствия для ставки описаны в [алгоритме](bidding-algorithm.md).

`Sync` ниже — фоновая задача чтения, а не мгновенная реплика всей WB. Её cursor — контрольная
точка страницы, до которой данные уже надёжно сохранены. Если процесс остановится, повторный
запуск продолжит с cursor и не потеряет обработанные факты. Это объясняет, почему система может
временно запретить применение новой ставки при stale data, но не должна уничтожать историю.
