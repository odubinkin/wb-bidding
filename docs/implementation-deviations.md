...
## Зачем фиксировать расхождения отдельно

Этот документ отделяет две вещи: осознанно реализованные ограничения безопасности и внешние
доказательства, которых нельзя получить без доступа или решения владельца. Он не описывает
работу системы с нуля — для этого есть [путеводитель](project-guide.md) и
[справочник реализации](implementation-reference.md). Его задача — не позволить локальным
проверкам выдать себя за подтверждение реального production-контракта.

Читайте статус вместе с [матрицей приёмочных доказательств](acceptance-evidence.md). `fail-closed`
не является незакрытым дефектом, когда запрет соответствует принятому рисковому контракту;
напротив, отсутствие внешнего evidence или решения может быть настоящим блокером выпуска.
